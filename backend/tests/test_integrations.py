...
def test_telegram_message_then_document_and_error():
    ...
    ch.send(
        OutboundMessage(
            recipients=["@c"], body="cap", attachments=[Attachment("f.txt", b"x", "text/plain")]
        )
    )
...
async def _owner(client, username):
    ...
    tok = (
        await client.post(
            "/api/v1/auth/login", data={"username": username, "password": "password123"}
        )
    ).json()["access_token"]
    h = {"Authorization": f"Bearer {tok}"}
    wsid = (
        await client.post("/api/v1/workspaces", json={"name": f"{username} WS"}, headers=h)
    ).json()["id"]
    return h, wsid
...
async def test_connection_crud_and_secret_masking(client):
    ...
    assert (
        await client.post(
            f"/api/v1/workspaces/{wsid}/connections",
            json={"type": "telegram", "name": "tg", "config": {}},
            headers=h,
        )
    ).status_code == 422
    assert (
        await client.post(
            f"/api/v1/workspaces/{wsid}/connections",
            json={"type": "slack", "name": "s", "config": {"x": "y"}},
            headers=h,
        )
    ).status_code == 422
    assert (
        await client.post(
            ...
            json={
                "type": "gmail",
                "name": "Reports",
                "config": {"email": "a@b.com", "app_password": "z"},
            },
            headers=h,
        )
    ).status_code == 409
    ...
    up = await client.patch(
        f"/api/v1/workspaces/{wsid}/connections/{cid}",
        json={"config": {"from_name": "Nightly", "app_password": ""}},
        headers=h,
    )
    ...
    assert (
        await client.delete(f"/api/v1/workspaces/{wsid}/connections/{cid}", headers=h)
    ).status_code == 200
...
async def test_action_step_delivers_via_connection(client, db_sync, monkeypatch):
    ...
    await client.post(
        f"/api/v1/workspaces/{wsid}/connections",
        json={"type": "telegram", "name": "tg-main", "config": {"bot_token": "T"}},
        headers=h,
    )
    ...
    wf = (
        await client.post(
            ...
            json={"name": "Notifier", "definition": definition},
            headers=h,
        )
    ).json()
    run = (
        await client.post(f"/api/v1/workspaces/{wsid}/workflows/{wf['id']}/trigger", headers=h)
    ).json()
    ...
    detail = (
        await client.get(
            f"/api/v1/workspaces/{wsid}/workflows/{wf['id']}/runs/{run['id']}", headers=h
        )
    ).json()
    step = detail["steps"][0]
...
async def test_action_step_without_connection_fails(client, db_sync):
    ...
    wf = (
        await client.post(
            f"/api/v1/workspaces/{wsid}/workflows",
            json={"name": "NoConn", "definition": definition},
            headers=h,
        )
    ).json()
    run = (
...
async def test_delivery_log_records_successful_send(client, db_sync, monkeypatch):
    ...
    await client.post(
        f"/api/v1/workspaces/{wsid}/connections",
        json={"type": "telegram", "name": "tg-log", "config": {"bot_token": "T"}},
        headers=h,
    )
    definition = (
        "steps:\n"
    ...
    wf = (
        await client.post(
            f"/api/v1/workspaces/{wsid}/workflows",
            json={"name": "Logger", "definition": definition},
            headers=h,
        )
    ).json()
    run = (
    ...
    wsd = (
        await client.get(f"/api/v1/workspaces/{wsid}/deliveries?status=delivered", headers=h)
    ).json()
...
async def test_delivery_log_records_failure(client, db_sync):
    ...
    wf = (
        await client.post(
            f"/api/v1/workspaces/{wsid}/workflows",
            json={"name": "FailLogger", "definition": definition},
            headers=h,
        )
    ).json()
    run = (
    ...
    feed = (
        await client.get(f"/api/v1/workspaces/{wsid}/deliveries?status=failed", headers=h)
    ).json()
    assert len(feed) >= 1
